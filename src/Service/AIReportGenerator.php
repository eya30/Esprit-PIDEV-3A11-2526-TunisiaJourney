...
class AIReportGenerator
{
    ...
    public function generateWeeklyReport(): array
    {
        try {
            ...
            $aiInsights = $this->generateAIInsights($voyagesData, $revenues, $bestVoyages, $worstVoyages);
    ...
    private function getVoyagesPerformance(): array
    {
        try {
            ...
            $avgOccupancyRate = $totalCapacityGlobal > 0 ? round(($totalOccupiedGlobal / $totalCapacityGlobal) * 100, 1) : 0;
    ...
    private function getBestVoyages(array $voyagesData): array
    {
        ...
        $voyagesWithRevenue = array_filter($voyages, function($v) {
            return ($v['paid_revenue'] ?? 0) > 0 || ($v['total_participants'] ?? 0) > 0;
        });
        usort($voyagesWithRevenue, function($a, $b) {
            return ($b['paid_revenue'] ?? 0) <=> ($a['paid_revenue'] ?? 0);
        });
        return array_slice($voyagesWithRevenue, 0, 5);
    }

    private function getWorstVoyages(array $voyagesData): array
    {
        $voyages = $voyagesData['voyages'] ?? [];
        $voyagesWithReservations = array_filter($voyages, function($v) {
            return ($v['total_reservations'] ?? 0) > 0;
        });
        usort($voyagesWithReservations, function($a, $b) {
            return ($a['occupancy_rate'] ?? 0) <=> ($b['occupancy_rate'] ?? 0);
        });
        return array_slice($voyagesWithReservations, 0, 5);
    }

    private function generateAIInsights(array $voyagesData, array $revenues, array $bestVoyages, array $worstVoyages): string
    {
        $summary = sprintf(
            "📊 RÉSUMÉ EXÉCUTIF\n" .
            "Cette semaine, %d voyages sont disponibles. " .
            "Nous avons enregistré %d réservations pour %d participants, " .
            "générant %.2f DT de revenus (%.2f DT payés, %.2f DT en attente). " .
            "Le taux d'occupation moyen est de %.1f%%.\n\n",
        ...
        if (!empty($bestVoyages)) {
            foreach (array_slice($bestVoyages, 0, 3) as $v) {
                $summary .= sprintf("• %s: %d participants, %.2f DT de revenus\n", 
                    $v['nom'], 
        ...
        $voyagesSansResa = count(array_filter($voyagesData['voyages'] ?? [], function($v) { 
            return ($v['total_reservations'] ?? 0) == 0; 
        }));
        ...
        if (($revenues['pending_revenue'] ?? 0) > 0) {
            $summary .= sprintf("• Revenus en attente: %.2f DT\n", $revenues['pending_revenue']);
        }
        $summary .= "\n";
        ...
        if (($voyagesData['avg_occupancy_rate'] ?? 0) < 50) {
            $summary .= "• Augmenter la visibilité des voyages sur les réseaux sociaux\n";
        }
...
/* CONTAINER (FORCE LEFT ALIGN) */
.container {
    width: 100%;
    padding: 15px 10px 15px 15px; /* plus compact + gauche */
    text-align: left;
}
...
<div class="header">
    ...
    <p>TunisiaJourney - Analyse des performances</p>
...
<tr>
<td>
<div class="value">' . ($report['summary']['total_voyages'] ?? 0) . '</div>
<div class="label">Voyages</div>
</td>
<td>
<div class="value">' . ($report['summary']['total_reservations'] ?? 0) . '</div>
<div class="label">Réservations</div>
</td>
<td>
<div class="value">' . ($report['summary']['total_participants'] ?? 0) . '</div>
<div class="label">Participants</div>
</td>
<td>
<div class="value">' . ($report['summary']['avg_occupancy_rate'] ?? 0) . '%</div>
...
<div class="revenue-value">' . number_format($report['summary']['total_revenue'] ?? 0, 2) . ' DT</div>
<div class="revenue-label">Total</div>
</div>

<div class="revenue-item">
<div class="revenue-value">' . number_format($report['summary']['paid_revenue'] ?? 0, 2) . ' DT</div>
<div class="revenue-label">Payé</div>
</div>

<div class="revenue-item">
<div class="revenue-value">' . number_format($report['summary']['pending_revenue'] ?? 0, 2) . ' DT</div>
<div class="revenue-label">En attente</div>
</div>
</div>
...
<div class="insights">
' . nl2br(htmlspecialchars($report['ai_insights'] ?? 'Aucun insight disponible')) . '
</div>

<!-- FOOTER -->
<div class="footer">
Rapport généré automatiquement - TunisiaJourney AI System
</div>
