...
class AdminProgrammeController extends AbstractController
{
    ...
    public function aiGenerate(Request $request, OllamaService $ollama): JsonResponse
    {
        ...
        if ($ollama->isAvailable()) {
            try {
                ...
                // Nettoyer la réponse pour ne garder que les activités
                if (strlen($activites) > 200) {
                    $activites = substr($activites, 0, 200);
                }
            } catch (\Exception $e) {
                $activites = '';
            }
    ...
    public function delete(Request $request, Connection $connection, string $id): Response
    {
        ...
        if ($this->isCsrfTokenValid('delete_programme_' . $id, $tokenString)) {
            ...
            if ($programme) {
                ...
                $imageFromDb = isset($programme['image']) && is_string($programme['image']) ? $programme['image'] : '';
                
                if (!empty($imageFromDb)) {
                    $imagePath = $uploadsDirPath . '/' . $imageFromDb;
    ...
    public function reservationsPdf(Connection $connection, string $id, Request $request): Response
    {
        ...
        if (!empty($projectDirPath)) {
            $chrootPath = realpath($projectDirPath);
            // FIX: realpath() returns string|false — after checking !== false, PHPStan
            // already knows it is a string, so the redundant is_string() is removed.
            if ($chrootPath !== false) {
