...
<<<<<<< HEAD
    ...
    public function new(Request $request, Connection $connection, ValidatorInterface $validator): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $capaciteRaw     = trim((string) $request->request->get('capaciteMax', ''));
            ...
            $evenement->setTitre($titreRaw);
            $evenement->setDescription($descriptionRaw);
            $evenement->setLieu($lieuRaw);
            ...
            if (empty($errors)) {
                ...
                if ($imageFile && $imageFile->getError() !== UPLOAD_ERR_NO_FILE) {
                    ...
                    // FIX :328 — getParameter() retourne mixed ; assert(is_string()) satisfait PHPStan
                    $uploadDirNew = $this->getParameter('uploads_directory');
                    assert(is_string($uploadDirNew));
                    $imageFile->move($uploadDirNew, $imageName);
                }
        ...
        // FIX :334 — $request->query->get() retourne mixed ; on force string avant createFromFormat
        $dateFromCalendar = $request->query->get('dateDebut', '');
        if ($dateFromCalendar && empty($old['dateDebut'])) {
            $d = \DateTime::createFromFormat('Y-m-d', (string) $dateFromCalendar);
            if ($d) {
                $old['dateDebut'] = $d->format('d/m/Y');
            }
        }
...
<<<<<<< HEAD
        ...
        if ($this->isCsrfTokenValid('delete_evenement_' . $id, is_string($token) ? $token : null)) {
            ...
            // FIX :482 — getParameter() retourne mixed ; assert(is_string()) satisfait PHPStan
            $uploadDir = $this->getParameter('uploads_directory');
            assert(is_string($uploadDir));
            if ($evenement && $evenement['Image']
                && file_exists($uploadDir . '/' . $evenement['Image'])) {
                unlink($uploadDir . '/' . $evenement['Image']);
            }
...
<<<<<<< HEAD
        ...
        $evenement = array_change_key_case($evenementRow, CASE_LOWER);
