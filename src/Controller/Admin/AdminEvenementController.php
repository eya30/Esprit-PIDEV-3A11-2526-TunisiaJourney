...
class AdminEvenementController extends AbstractController
{
    ...
    // ── Parse une date saisie en JJ/MM/AAAA ou AAAA-MM-JJ ────────────────────
    ...
    // ── Normalise un DateTime vers Y-m-d pour la BDD ─────────────────────────
    private function normalizeDate(?\DateTime $date): ?string
    {
        return $date ? $date->format('Y-m-d') : null;
    }

    // ── Convertit le ConstraintViolationList en tableau [propertyPath => msg] ─
    private function buildErrorsArray(iterable $violations): array
    {
        $errors = [];
        foreach ($violations as $violation) {
            $path = $violation->getPropertyPath();
            // Convertir les noms de propriétés pour correspondre aux noms des champs du formulaire
            $path = match($path) {
                'Titre' => 'titre',
    ...
    public function index(Connection $connection, Request $request): Response
    {
        ...
        if ($search !== '') {
            ...
            // Vérifier si la recherche ressemble à une date (JJ/MM/AAAA ou JJ/MM/AA)
            $isDateSearch = preg_match('#^\d{2}/\d{2}/\d{2,4}$#', $search);
            
            if ($isDateSearch) {
                // Convertir la date recherchée au format YYYY-MM-DD pour la BDD
        ...
        $tousEvenements = array_map(
            fn($row) => array_change_key_case($row, CASE_LOWER),
            $connection->fetchAllAssociative(
                "SELECT IDEv, Titre, DateDebut, DateFin, Lieu FROM Evenement ORDER BY DateDebut ASC"
            )
        );

        return $this->render('admin/evenement/admin_evenement_index.html.twig', [
            ...
            'search'          => $search,
            'sort'            => $sort,
            'tous_evenements' => $tousEvenements,
        ]);
    }
    ...
    public function exportPdf(Connection $connection, Request $request): Response
    {
        ...
        if ($search !== '') {
            ...
            // Vérifier si la recherche ressemble à une date
            $isDateSearch = preg_match('#^\d{2}/\d{2}/\d{2,4}$#', $search);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PAGE ACTIVITÉS - VERSION CORRIGÉE
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public function activites(Connection $connection, int $id, Request $request): Response
    {
        ...
        if ($search !== '') {
            ...
            $params = array_merge($params, [$searchLower, $searchLower, $searchLower, $searchLower, $searchLower, $searchLower]);
        }

        $sql = "SELECT a.* FROM Activite a WHERE a.IDEv = ? $where";
        $activitesRaw = $connection->fetchAllAssociative($sql, $params);
        ...
        $activitesPaginated = array_slice($activites, $offset, $itemsPerPage);

        return $this->render('admin/evenement/activites.html.twig', [
            'evenement' => $evenement,
            'activites' => $activitesPaginated,
            'search' => $search,
            'sort' => $sort,
            'current_page' => $page,
            'total_pages' => $totalPages,
