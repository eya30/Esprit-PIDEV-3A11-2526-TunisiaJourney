...
class ReservationApiController extends AbstractController
{
    ...
    #[Route('/reservations/voyage/{idV}', name: 'api_reservations_by_voyage', methods: ['GET'])]
    public function getReservationsByVoyage(Connection $connection, int $idV): JsonResponse
    {
        // Retourne uniquement les réservations du user connecté pour ce voyage (idV)
        $user = $this->getUser();
        if (!$user) {
            return $this->json([]);
        }
        ...
        $reservations = $connection->fetchAllAssociative(
            "SELECT r.* FROM reservationprog r
             JOIN programmes p ON r.idP = p.idProg
             WHERE p.idV = :idV AND r.email = :email
