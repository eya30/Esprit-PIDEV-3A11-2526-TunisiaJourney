"""
predict.py
Recoit une liste d'avis (JSON) en argument, retourne l'analyse ML en JSON.
Appele par Symfony via Process.

Usage :
    python predict.py '[{"note":5,"commentaire":"Super activite !"}]'
"""
...
try:
    ...
    print(json.dumps({
        "success": False,
        "error": "Modele introuvable. Lancez d'abord train_model.py"
    }))
    sys.exit(1)
...
LABEL_FR = {0: "negatif", 1: "neutre", 2: "positif"}
...
for avis in avis_list:
    ...
    note_text = f"note {note} etoiles. " if note else ""
...
if global_sentiment == "positive":
    if avg >= 4.5:
        opening = "Excellente experience !"
    elif avg >= 4.0:
        opening = "Tres bonne experience !"
    else:
        opening = "Les participants sont globalement satisfaits"
elif global_sentiment == "negative":
    opening = "Des points d'amelioration sont signales"
else:
    opening = "Avis partages"

summary = f"{opening} ({avg}/5 sur {total} avis)"
summary += f" - {pos_pct}% positifs, {neg_pct}% negatifs."

if avg >= 4.0:
    summary += " Fortement recommande !"
elif avg >= 3.0:
    summary += " Recommande avec quelques reserves."
elif avg >= 2.0:
    summary += " A considerer selon vos attentes."
else:
    summary += " A eviter selon les retours."

if confidence >= 70:
    summary += f" (analyse ML fiable, {total} avis)"
elif total < 5:
    summary += " (avis encore limites)"
